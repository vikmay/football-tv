...
- **Key invariant**: A match object with `score` and `status === "Match Finished"` can never be replaced by a version without a score or with a non-finished status, *unless* the scores differ (to fix stale 0-0).
...
3. If cache expired or forced → launches parallel fetches:
    ...
    - World Cup standings: `fetchWorldCup2026GroupStandings()` is **defined but never called** in the `Promise.all` (see bug below)
...
8. World Cup group standings stored under `"Таблиця ЧС 2026"` — always empty due to the bug
...
The function `getCupFallbackEvents()` contains hardcoded semifinal matches for the 2025-2026 Ukrainian Cup season. These are used when the uaf.ua website is unreachable. The fallback events are filtered by date window before use.

### MLB filtering
The World Cup section filters out MLB team names because Flashscore sometimes mis-categorizes baseball matches under "world-championship". The `nonSoccerTeams` set contains all 30 MLB team names.
...
2. **"00:00" times in World Cup matches** — No time-filtering for World Cup, so entries with unknown kickoff times appear with `"time": "00:00"`.
3. **World Cup match duplicates** — Same match appears with `"time": "00:00"` and with a real time (e.g., México – South Africa appears twice). The frontend duplicates prefer scores, but since none have scores, both entries may display.
4. **No `package.json`** — The project has no npm dependencies, but a `package.json` would clarify runtime requirements.
